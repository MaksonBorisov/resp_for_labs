...
function addPlusOrMinus() {
  ...
  if (calculatorDisplay.textContent.includes('') && !calculatorDisplay.textContent.includes('-')) {
...
const calculate = {
  ...
  '*': (firstNumber, secoundNumber) => Number(firstNumber * secoundNumber).toFixed(2),
  '+': (firstNumber, secoundNumber) => Number(firstNumber + secoundNumber).toFixed(2),
  '-': (firstNumber, secoundNumber) => Number(firstNumber - secoundNumber).toFixed(2),
...
function useOperator(operator) {
  ...
  if (!firstValue) {
    ...
    const calculation = calculate[operatorValue](Number(firstValue), currentValue);
...
function deleteData() {
  if( calculatorDisplay.textContent && calculatorDisplay.textContent != "0") {
    calculatorDisplay.textContent = String(calculatorDisplay.textContent).slice(0, -1);
